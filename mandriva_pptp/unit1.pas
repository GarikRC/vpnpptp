...
//перезаписываем скрипт опускания соединения ip-down пустым
 ...
 //перезаписываем скрипт опускания соединения /opt/vpnpptp/ip-down пустым если кто-то захочет там писать маршруты при опускании ppp0
 Shell('rm -f /opt/vpnpptp/ip-down');
 Memo_ip_down.Clear;
 Memo_ip_down.Lines.Add('#!/bin/sh');
 Memo_ip_down.Lines.SaveToFile('/opt/vpnpptp/ip-down');
 Shell('chmod a+x '+'/opt/vpnpptp/ip-down');

//Записываем готовый конфиг
 If Mii_tool_no.Checked then mii_tool:='mii-tool-no' else mii_tool:='mii-tool-yes';
