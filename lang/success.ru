...
2) Запустив ponoff (Управление соединением VPN PPTP/L2TP) из Меню запуска приложений;
...
5) Запустив xl2tpd как сервис, и выполнив в консоли под администратором команду echo "c имя_соединения" > /var/run/xl2tpd/l2tp-control (в этом случае VPN L2TP будет работать без графического интерфейса) для соединения, а для отключения - команду echo "d имя_соединения" > /var/run/xl2tpd/l2tp-control.

Данный конфигуратор vpnpptp (Настройка соединения VPN PPTP/L2TP) можно вызвать в консоли под администратором командой vpnpptp или из Меню запуска приложений, или из Центра Управления Вашего дистрибутива (например, раздел Сеть и Интернет->Настройка VPN-соединений->VPN PPTP/L2TP).
...
Более подробную информацию и сопроводительную документацию о программе можно найти в папке /opt/vpnpptp/wiki/
